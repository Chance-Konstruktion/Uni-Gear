...
bl_info = {
    ...
    "description": "Evolventenzahnrad mit Schraegverzahnung, Nabe, Stacking, Kegelverzahnung, "
                   "Innenverzahnung, Zahnrad-Paarung und DIN-3960-Modus.",
    "category": "Add Mesh",
}
...
def build_trochoidal_fillet(pitch_radius, teeth, pressure_angle, root_radius,
                            base_radius, flank_start_pt, fillet_samples=8):
    """
    Erzeugt einen trochoidalen Zahnfuss-Verlauf (DIN-3960-konform).
    Die Trochoide entsteht durch das Abrollen des Gegenrad-Kopfkreises
    auf dem Teilkreis. Naeherung ueber die Rollbewegung eines Punktes
    am Kopfkreisradius des Bezugsprofil-Werkzeugs (Zahnstange: r -> inf).
    Fuer endliche Raeder ist dies eine sehr gute Approximation.
    """
    ...
    # Werkzeug-Kopfhoehe (Zahnstange h_aP* = 1.25 inkl. Spiel)
    h_tool = 1.25 * module
    # Rollkreisradius = Teilkreisradius
    r_roll = pitch_radius
    ...
    for i in range(1, fillet_samples + 1):
        u = i / (fillet_samples + 1)
        # Trochoidale Interpolation: kombiniert Rollbewegung mit radialer Absenkung.
        # Erzeugt die charakteristische hinterschnittene Kurve bei kleinen z.
        phi = u * math.pi * 0.5
        s_radial = math.sin(phi)
        s_tangential = 1.0 - math.cos(phi)
        # Radialer Anteil: vom Fusskreis zum Evolventenstart
        r_current = root_radius + (inv_radius - root_radius) * s_radial
        # Tangentialer Anteil: leichte Verschiebung durch Rollbewegung
        angle_shift = s_tangential * (h_tool / r_roll) * 0.3
        ang = inv_angle + angle_shift
        x = r_current * math.cos(ang)
        y = r_current * math.sin(ang)
        pts.append((x, y))
    return pts
...
def build_gear_profile(
    ...
    use_trochoidal: Wenn True, wird ein trochoidaler Zahnfuss erzeugt (DIN 3960).
    """
    if teeth < 4:
    ...
    # Mit Profilverschiebung x:
    #   Kopfkreis:  r_a = r + (h_aP* + x) * m
    #   Fusskreis:  r_f = r - (h_fP* - x) * m
    x = profile_shift
    ...
    # Halbe Zahndickenwinkel auf dem Teilkreis (mit Profilverschiebung).
    # s = m * (pi/2 + 2*x*tan(alpha))  ->  half_tooth_angle = s / (2*r)
    ...
    if use_trochoidal:
        raw_fillet = build_trochoidal_fillet(
            pitch_radius, teeth, pressure_angle, root_radius,
            base_radius, raw_flank[0], fillet_samples)
    else:
...
def build_internal_gear_profile(
    ...
    """
    Erzeugt das geschlossene 2D-Profil der Innenverzahnung (Hohlrad).
    Gibt die innere Kontur (Zahnprofil) als Liste von (x, y)-Punkten
    gegen den Uhrzeigersinn zurueck.

    Bei einem Hohlrad zeigen die Zaehne nach innen:
      - Kopfkreis (tip) liegt INNERHALB des Teilkreises
      - Fusskreis (root) liegt AUSSERHALB des Teilkreises
    """
    if teeth < 4:
        raise ValueError("Zaehnezahl muss >= 4 sein.")
    ...
    x = profile_shift
    # Innenverzahnung: Kopf nach innen, Fuss nach aussen
    tip_radius = pitch_radius - (addendum_coeff + x) * module
    root_radius = pitch_radius + (dedendum_coeff - x) * module

    if tip_radius <= 0.0:
        raise ValueError("Innenverzahnung: Teilkreisradius zu klein fuer diese Zaehnezahl/Modul.")

    angular_pitch = 2.0 * math.pi / teeth
    # Zahndicke der Innenverzahnung auf dem Teilkreis (Zahnluecke des Hohlrads = Zahn des Gegenrads)
    half_space_at_pitch = (math.pi / 2.0 - 2.0 * x * math.tan(pressure_angle)) / teeth
    flank_offset = half_space_at_pitch + inv(pressure_angle)

    # Evolvente vom Grundkreis (oder Kopfkreis, falls dieser groesser) bis zum Fusskreis (aussen).
    raw_flank = build_tooth_flank(base_radius, tip_radius, root_radius, flank_samples)
    # Fillet am Kopfkreis (innerer Rand) — von Evolventenstart zum Kopfkreis.
    raw_fillet = build_root_fillet(raw_flank[0], tip_radius, fillet_samples)

    # Flanken-Orientierung: fuer die Innenverzahnung zeigt die Zahnluecke
    # (Ausbuchtung nach aussen = root) auf die Zahnmitte, der Zahnkopf (tip, innen)
    # liegt zwischen den Zaehnen.
    ...
    # Polarwinkel am Fusskreis (aussen, groesster Radius).
    right_root_angle = math.atan2(right_flank[-1][1], right_flank[-1][0])
    left_root_angle = -right_root_angle

    # Polarwinkel am Kopfkreis (innen, kleinster Radius).
    if right_fillet:
        right_tip_pt = right_fillet[-1]
    else:
        right_tip_pt = right_flank[0]
    ...
    for i in range(teeth):
        center_angle = i * angular_pitch
        # Fusskreisbogen (aussen) vom vorherigen Zahn zum aktuellen.
        prev_left_root_world = (i - 1) * angular_pitch + left_root_angle
        curr_right_root_world = center_angle + right_root_angle
        for px, py in build_arc(root_radius, prev_left_root_world, curr_right_root_world, root_samples):
            profile.append((px, py))

        # Rechte Flanke (von aussen/root nach innen/tip) — reversed, da raw_flank
        # von innen nach aussen laeuft.
        for px, py in reversed(right_flank):
            xr, yr = rotate_xy(px, py, center_angle)
            profile.append((xr, yr))

        # Rechter Fillet (Evolventenstart -> Kopfkreis), reversed.
        for px, py in reversed(right_fillet):
            xr, yr = rotate_xy(px, py, center_angle)
            profile.append((xr, yr))

        # Kopfkreisbogen (innen) von rechtem Fillet-Ende zum linken Fillet-Ende.
        tip_start = center_angle + right_tip_angle
        tip_end = center_angle + left_tip_angle
        for px, py in build_arc(tip_radius, tip_start, tip_end, tip_samples):
            profile.append((px, py))

        # Linker Fillet (Kopfkreis -> Evolventenstart).
        for px, py in left_fillet:
            xr, yr = rotate_xy(px, py, center_angle)
            profile.append((xr, yr))

        # Linke Flanke (von innen/tip nach aussen/root).
...
def create_internal_gear_mesh(
    ...
    """Erstellt ein Hohlrad (Internal Gear / Ring Gear).

    Das Hohlrad ist ein Ring mit Innenverzahnung. Die Zaehne zeigen nach innen.
    ring_outer_radius:  Aussenradius des Ringkoerpers.
    """
    profile_2d, root_radius_int, tip_radius_int = build_internal_gear_profile(
        pitch_radius=pitch_radius,
        teeth=teeth,
        pressure_angle_deg=pressure_angle_deg,
        profile_shift=profile_shift,
    ...
    if ring_outer_radius <= root_radius_int * 1.02:
        raise ValueError(
            f"Ring-Aussenradius ({ring_outer_radius:.4f}) muss groesser als "
            f"der Fusskreisradius der Innenverzahnung ({root_radius_int:.4f}) sein.")
    ...
    # --- Innenprofil (Zahnprofil, helical) ---
    inner_layers = []
    for k in range(layers + 1):
        z   = thickness * k / layers
        phi = z * tan_helix / pitch_radius
    ...
    # --- Stirnflaechen ---
    def _fill_ring_cap(outer_loop, inner_loop):
        loops = [outer_loop, inner_loop]
        bmesh.ops.triangle_fill(bm, edges=_collect_loop_edges(bm, loops), use_beauty=True)

    _fill_ring_cap(ring_bottom, inner_bottom)
    _fill_ring_cap(ring_top,    inner_top)
...
class GearGeneratorProperties(bpy.types.PropertyGroup):
    ...
    use_internal: bpy.props.BoolProperty(
        ...
        description="Hohlrad mit nach innen gerichteten Zaehnen erzeugen",
        default=False,
    )
    internal_ring_diameter: bpy.props.FloatProperty(
        name="Ring-Aussendurchmesser",
        description="Aussendurchmesser des Ringkoerpers",
        default=0.030, min=0.004, max=4.0, unit="LENGTH",
    )

    # --- Zahnrad-Paarung (Gear Pairing) ---
    use_pairing: bpy.props.BoolProperty(
        name="Zahnrad-Paarung",
        description="Zweites Zahnrad passend zum ersten erzeugen (korrekter Achsabstand)",
        default=False,
    )
    pair_teeth: bpy.props.IntProperty(
        name="Zaehnezahl Gegenrad",
        description="Zaehnezahl des zweiten Zahnrads",
        default=16, min=6, max=200,
    )
    pair_thickness: bpy.props.FloatProperty(
        name="Dicke Gegenrad",
        description="Axiale Dicke des zweiten Zahnrads",
        default=0.005, min=0.0001, max=1.0, unit="LENGTH",
    )
    pair_internal: bpy.props.BoolProperty(
        name="Gegenrad als Hohlrad",
        description="Das Gegenrad als Innenverzahnung (Hohlrad) erzeugen",
        default=False,
    )
    ...
    use_din3960: bpy.props.BoolProperty(
        name="DIN-3960-Modus",
        description="Genormte Module nach DIN 780, Profilverschiebung und trochoidaler Zahnfuss",
        default=False,
    )
    din_module: bpy.props.EnumProperty(
        name="Modul (DIN 780)",
        description="Genormter Modul nach DIN 780 Reihe 1",
        items=[
            ("0.5",   "0,5 mm",   "Modul 0,5 mm"),
            ("0.6",   "0,6 mm",   "Modul 0,6 mm"),
            ("0.7",   "0,7 mm",   "Modul 0,7 mm"),
            ("0.8",   "0,8 mm",   "Modul 0,8 mm"),
            ("0.9",   "0,9 mm",   "Modul 0,9 mm"),
            ("1.0",   "1,0 mm",   "Modul 1,0 mm"),
            ("1.25",  "1,25 mm",  "Modul 1,25 mm"),
            ("1.5",   "1,5 mm",   "Modul 1,5 mm"),
            ("2.0",   "2,0 mm",   "Modul 2,0 mm"),
            ("2.5",   "2,5 mm",   "Modul 2,5 mm"),
            ("3.0",   "3,0 mm",   "Modul 3,0 mm"),
            ("4.0",   "4,0 mm",   "Modul 4,0 mm"),
            ("5.0",   "5,0 mm",   "Modul 5,0 mm"),
            ("6.0",   "6,0 mm",   "Modul 6,0 mm"),
            ("8.0",   "8,0 mm",   "Modul 8,0 mm"),
            ("10.0",  "10,0 mm",  "Modul 10,0 mm"),
        ],
        default="1.0",
    )
    din_profile_shift: bpy.props.FloatProperty(
        name="Profilverschiebung x",
        description="Profilverschiebungsfaktor (DIN 3960). 0 = Nullrad, positiv = dickerer Zahnfuss",
        default=0.0, min=-0.5, max=0.8,
    )
...
class MESH_OT_create_gear(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # --- DIN-3960-Modus: Modul bestimmt Teilkreisdurchmesser ---
        if props.use_din3960:
            din_m = float(props.din_module) * 0.001  # mm -> m (Blender-Einheit)
            pitch_r = din_m * props.teeth * 0.5
        ...
        if props.use_pairing and not props.use_stack:
            module = 2.0 * pitch_r / props.teeth
            pair_pitch_r = module * props.pair_teeth * 0.5

            if props.pair_internal:
                # Innenpaarung: Achsabstand = r_hohlrad - r_ritzel
                center_dist = pair_pitch_r - pitch_r
                ring_outer_r_pair = pair_pitch_r * 1.5
                # Phasenversatz: halber Zahnwinkel fuer korrektes Ineinandergreifen
                phase = math.pi / props.pair_teeth
                try:
                    obj2 = create_internal_gear_mesh(
                        pitch_radius=pair_pitch_r,
                        teeth=props.pair_teeth,
                        thickness=props.pair_thickness,
                        pressure_angle_deg=props.pressure_angle,
                        ring_outer_radius=ring_outer_r_pair,
                        helix_angle_deg=-helix_deg,  # Gegenrichtung bei Helical
                        profile_shift=profile_shift,
                    )
                    obj2.location.x = center_dist
                    obj2.rotation_euler.z = phase
                except Exception as exc:
                    self.report({"ERROR"}, f"Gegenrad (Hohlrad): {exc}")
                    return {"CANCELLED"}
            else:
                # Aussenpaarung: Achsabstand = r1 + r2
                center_dist = pitch_r + pair_pitch_r
                # Phasenversatz: halber Zahnwinkel + halber Lueckenwinkel
                phase = math.pi / props.pair_teeth
                try:
                    obj2 = create_gear_mesh(
                        pitch_radius=pair_pitch_r,
                        teeth=props.pair_teeth,
                        thickness=props.pair_thickness,
                        pressure_angle_deg=props.pressure_angle,
                    ...
                    self.report({"ERROR"}, f"Gegenrad: {exc}")
                    return {"CANCELLED"}

            label = f"Zahnradpaarung {props.teeth}Z / {props.pair_teeth}Z"
            self.report({"INFO"}, f"{label} erstellt (Achsabstand {center_dist*1000:.2f} mm).")
            return {"FINISHED"}
...
class VIEW3D_PT_gear_generator(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        # --- DIN-3960-Modus ---
        box = layout.box()
        box.prop(props, "use_din3960", icon="PREFERENCES")
        col = box.column()
        col.enabled = props.use_din3960
        col.prop(props, "din_module")
        col.prop(props, "din_profile_shift")
        # Berechneter Teilkreisdurchmesser anzeigen
        if props.use_din3960:
            din_m = float(props.din_module)
            d_calc = din_m * props.teeth
            col.label(text=f"Teilkreis-\u00d8: {d_calc:.2f} mm")
            # Unterschnitt-Warnung
            alpha_rad = math.radians(props.pressure_angle)
            z_min = 2.0 / (math.sin(alpha_rad) ** 2)
            x_min = (z_min - props.teeth) / z_min  # Mindest-Profilverschiebung
            if props.teeth < z_min and props.din_profile_shift < x_min:
                row = col.row()
                row.alert = True
                row.label(text=f"\u26a0 Unterschnitt! (z < {z_min:.0f}, x < {x_min:.2f})", icon="ERROR")

        # --- Zahnrad ---
        box = layout.box()
        box.label(text="Zahnrad", icon="MESH_CIRCLE")
        # Im DIN-Modus wird der Teilkreisdurchmesser automatisch berechnet.
        sub = box.column()
        sub.enabled = not props.use_din3960
        sub.prop(props, "pitch_diameter")
        ...
        # --- Innenverzahnung (Hohlrad) ---
        box = layout.box()
        box.enabled = not props.use_bevel
        box.prop(props, "use_internal", icon="MESH_TORUS")
        col = box.column()
        ...
        box = layout.box()
        box.enabled = not props.use_internal
        box.prop(props, "use_bevel", icon="CONE")
        col = box.column()
        col.enabled = props.use_bevel and not props.use_internal
        ...
        # Im Kegelrad- oder Hohlrad-Modus: bestimmte Sektionen deaktivieren.
        cylindrical_enabled = not props.use_bevel and not props.use_internal

        # Schraegverzahnung (auch bei Hohlrad verfuegbar)
        box = layout.box()
        box.enabled = not props.use_bevel
        box.prop(props, "use_helical", icon="MOD_SCREW")
        col = box.column()
        col.enabled = not props.use_bevel and props.use_helical
        ...
        # Zentrische Bohrung (auch beim Kegelrad verfuegbar, nicht beim Hohlrad)
        box = layout.box()
        box.enabled = not props.use_internal
        box.prop(props, "use_bore", icon="HANDLE_ALIGN")
        col = box.column()
        col.enabled = props.use_bore and not props.use_internal
        ...
        # Stacking
        box = layout.box()
        box.enabled = cylindrical_enabled and not props.use_pairing
        box.prop(props, "use_stack", icon="DUPLICATE")
        col = box.column()
        col.enabled = cylindrical_enabled and props.use_stack and not props.use_pairing
        ...
        # --- Zahnrad-Paarung ---
        box = layout.box()
        box.enabled = cylindrical_enabled and not props.use_stack
        box.prop(props, "use_pairing", icon="MOD_MIRROR")
        col = box.column()
        col.enabled = props.use_pairing and cylindrical_enabled and not props.use_stack
        col.prop(props, "pair_teeth")
        col.prop(props, "pair_thickness")
        col.prop(props, "pair_internal")
        # Berechneten Achsabstand anzeigen
        if props.use_pairing and not props.use_bevel and not props.use_internal:
            if props.use_din3960:
                din_m_val = float(props.din_module) * 0.001
                pr1 = din_m_val * props.teeth * 0.5
            else:
                pr1 = props.pitch_diameter * 0.5
            module_val = 2.0 * pr1 / props.teeth
            pr2 = module_val * props.pair_teeth * 0.5
            if props.pair_internal:
                a = (pr2 - pr1) * 1000.0
            else:
                a = (pr1 + pr2) * 1000.0
            col.label(text=f"Achsabstand: {a:.2f} mm")
